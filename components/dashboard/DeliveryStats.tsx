  ...
  return (
    <Card>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          <p className="text-sm text-ki-text-muted mb-1">{t("todayOrders")}</p>
          <p className="text-3xl font-bold font-mono text-ki-text">
            {formatNumber(stats.todayOrders, locale)}
          </p>
        </div>
        <div>
          <p className="text-sm text-ki-text-muted mb-1">{t("weeklyAvg")}</p>
          <p className="text-3xl font-bold font-mono text-ki-text-secondary">
      ...
      {/* 핫스팟 위치 */}
      <div className="border-t border-ki-border pt-4">
        <div className="flex items-center gap-2 mb-3">
          <MapPin className="h-4 w-4 text-ki-text-muted" />
          <span className="text-base text-ki-text-secondary">{t("topLocations")}</span>
        </div>
          ...
          {stats.topLocations.slice(0, 5).map((location, index) => (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-sm text-ki-text-muted w-4">{index + 1}.</span>
                <span className="text-base text-ki-text-secondary">{location.name}</span>
              </div>
              <span className="text-base font-mono text-ki-text-secondary">
