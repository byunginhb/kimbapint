    ...
    <div className="relative">
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-center relative z-10">
        ...
        <div className="flex-1 min-w-0 text-center lg:text-left order-1">
          <p className="text-xs text-ki-text-muted tracking-[0.2em] font-mono mb-3">
            {t("geoIndex")}
          </p>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-bold mb-4 lg:mb-6">
            <span className="text-ki-text">{t("status")}: </span>
            <span className={statusColor}>{status}</span>
          </h2>

          <button className="mx-auto lg:mx-0 flex items-center justify-center w-12 h-12 border border-ki-border-subtle rounded-lg mb-4 lg:mb-6 hover:bg-ki-elevated transition-colors">
            <svg viewBox="0 0 24 24" className="w-5 h-5 text-ki-text" fill="currentColor">
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
            </svg>
          </button>

          <p className="text-ki-text-secondary text-sm lg:text-base mb-6 lg:mb-8 max-w-md mx-auto lg:mx-0 leading-relaxed">
            {t("description")}
          </p>
...
function HighestRiskCard({ market, maxRiskLabel }: { market: HighestRiskMarket; maxRiskLabel: string }) {
  return (
    <div className="bg-ki-surface-alt/80 border border-ki-border-subtle rounded-xl p-4">
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-lg overflow-hidden bg-ki-elevated flex items-center justify-center text-4xl">
          {market.countryFlag}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-yellow-500 text-sm">⚠</span>
            <span className="text-xs text-ki-text-muted tracking-wider font-mono">
              {maxRiskLabel}
            </span>
          </div>
          <p className="text-sm text-ki-text leading-tight line-clamp-2">
            {market.title}
          </p>
        </div>
        <div className="flex items-center gap-2 pl-4 border-l border-ki-border-subtle">
...
function DoomsdayGauge({ percentage, labels }: { percentage: number; labels: { nothing: string; maybe: string; happening: string; itsOver: string } }) {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute bottom-4 left-0 right-0 flex justify-between px-8 text-ki-text-muted text-xs font-mono">
        <span>0</span>
