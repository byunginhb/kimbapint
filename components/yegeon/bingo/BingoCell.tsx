...
export default function BingoCell({ cell, index, isEditing }: BingoCellProps) {
  const isFreeSpace = index === FREE_SPACE_INDEX

  if (isFreeSpace) {
    return (
      <div className={`yegeon-bingo-cell yg-bg-bingo-free flex flex-col items-center justify-center gap-0.5 rounded border yg-border-bingo-free${isEditing ? " yegeon-bingo-cell-fixed" : ""}`}>
  ...
  if (isEditing) {
    return (
      <div className={`yegeon-bingo-cell yegeon-bingo-cell-editing flex flex-col justify-between rounded ${borderClass}`}>
        {content}
      </div>
    )
  }
